...
import math
from agents.A3C import Model as A3C_Model
    ...
    def compute_intrinsic_reward(self, rollout):
        ...
        for i in minibatches:
          ...
          with torch.no_grad():
            ...
            icm_obs = phi[self.config.num_agents:]
            obs_diff = icm_obs_pred - icm_obs

            error = obs_diff.pow(2).sqrt()
            dim0, dim1 = icm_obs.shape
            intr_reward = error
            for i in range(dim0):
                if i==0:
                    self.diff = icm_obs[0]
                else:
                    diff=icm_obs[i]-icm_obs[i-1]
                    self.diff += diff.pow(2).sqrt()

                if torch.mean(error[i]-self.diff)<=0:
                    intr_reward[i]=self.config.icm_prediction_beta*error[i]
                else:
                    intr_reward[i] = -self.config.icm_prediction_beta*error[i]+2*self.config.icm_prediction_beta*self.diff
            #else:
            intr_reward = intr_reward.sum(dim=1)
